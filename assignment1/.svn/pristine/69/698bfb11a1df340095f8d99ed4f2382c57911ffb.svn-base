...
int main(int argc, char *argv[])
{
  ...
  // I'm assuming we're meant to use QLEN (= 6) from config.h for the max number of queued connections
  ...
  while (listen_socket >= 0)
  {
    ...
    connection_socket = accept(listen_socket, (struct sockaddr *)&client_address, (socklen_t*)&sockaddr_in_size);
    ...
    if ((pid = fork()) == 0)
    {
      ...
      if (Parse_HTTP_Request(connection_socket, &new_request) == false){
        printf("Couldn't parse HTTP request\n");
        status_code = 400;
      ...
      // If the HTTP request couldn't be parsed, the "Bad Request" response has already been set
      if (status_code != 400){

      // Requested resource is found
      // HEAD requests just don't send the actual file content itself, but still need to know it exists...
        if (strncmp(new_request.method, "GET", 4) == 0 || strncmp(new_request.method, "HEAD", 5) == 0){
          
          // Check if the file exists
          if (Is_Valid_Resource(new_request.URI) == true){
            status_code = 200;
            status_phrase = "OK";
          } else {
            status_code = 404;
        ...
        // The HTTP method isn't implemented
        } else if (strncmp(new_request.method, "POST", 10) == 0 || strncmp(new_request.method, "PUT", 10) == 0 || strncmp(new_request.method, "DELETE", 10) == 0 || strncmp(new_request.method, "CONNECT", 10) == 0 || strncmp(new_request.method, "OPTIONS", 10) == 0 || strncmp(new_request.method, "TRACE", 10) == 0 || strncmp(new_request.method, "PATCH", 10) == 0){
          status_code = 501;
      ...
      // When we are happy to send a response, (200 code)
      // ...and the client is happy to receive the resource body, (GET request, not HEAD)
      // ...we can send the response
      is_ok_to_send_resource = status_code == 200 && strncmp(new_request.method, "GET", 4) == 0;
      /* END CODE SNIPPET 10 */
      ...
      else
      {
        ...
        if (strncmp(new_request.method, "HEAD", 5) == 0){
          // Copied Send_Resource() in helpers.c, but modified it to not send the body, in this new function
