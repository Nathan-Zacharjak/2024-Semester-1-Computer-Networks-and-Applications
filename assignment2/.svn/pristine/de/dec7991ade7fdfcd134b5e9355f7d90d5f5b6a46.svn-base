...
int ComputeChecksum(struct pkt packet)
{
  ...
  /* First check the sequence number and ACK number are OK, by using them in the checksum*/
  checksum = checksum + packet.seqnum;
  checksum = checksum + packet.acknum;

  /* Now check each bit of the message payload is OK, the payload's length is always 20 bits */
  for (i = 0; i < 20; i++){
    /* C characters are just integers from 0-255, so we can just add them like integers to the checksum */
...
static int A_lastacknum; /* the sequence number expected next by the sender */
...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    /* Timer is set to 15.0 as recommended by the assignment description */
    starttimer(A, 15.0);
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (A_lastacknum != packet.acknum) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      A_lastacknum = packet.acknum;  /* record the last number we ACKed */
...
void A_init(void)
{
  ...
  windowcount = 0;
  A_lastacknum = 1; /* Pretend initially the last ACKed packet was 1, since we start by sending sequence number 0, so if the first packet is corrupt, B will send ACK 1 back, saying to re-send packet 0 */
}
...
static int B_lastsentack;   /* the last sent ACK number by B */
